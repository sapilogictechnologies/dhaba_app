...
const AdminOrdersPage = () => {
  ...
  const closeModal = () => {
    setModal(null);
    setEtaValue("");
    setCancelForm({ reason: "", reasonText: "" });
    ...
    <section className="shell">
      ...
      <div className="order-grid">
        {orders.map((order) => (
          <article
            key={order._id}
            className={`order-card ${order.status === "ACCEPTED" ? "new-order" : order.status === "PREPARING" ? "preparing" : order.status === "READY" ? "ready" : ""}`}>
            <div className="order-header">
              <div
                style={{ display: "flex", gap: "0.35rem", flexWrap: "wrap" }}>
                <span className={`badge source-${order.source}`}>
                ...
                {order.tableNumber && (
                  <span className="badge badge-blue">T{order.tableNumber}</span>
                )}
              </div>
            ...
            {order.payment.proofImageUrl && (
              <a
                href={`${apiBase}${order.payment.proofImageUrl}`}
                target="_blank"
                rel="noreferrer"
            ...
            <div className="order-actions">
              {["PLACED", "UNDER_REVIEW"].includes(order.status) && (
                <button
                  ...
                  onClick={() =>
                    act(
                      () => acceptOrder(order._id).unwrap(),
                      `${order.orderNo} accepted`,
                    )
                  }>
                  ✓ Accept
                </button>
              ...
              {["PLACED", "UNDER_REVIEW"].includes(order.status) && (
                <button
                  className="danger sm"
                  onClick={() => setModal({ type: "reject", order })}>
                  ✗ Reject
                </button>
              )}
              {order.payment.paymentStatus === "UNDER_REVIEW" && (
                <button
                  ...
                  onClick={() =>
                    act(
                      () => verifyPayment(order._id).unwrap(),
                      "Payment verified",
                    )
                  }>
                  💳 Verify Pay
              ...
              {order.payment.paymentStatus === "UNDER_REVIEW" && (
                <button
                  className="danger sm"
                  onClick={() => setModal({ type: "rejectPayment", order })}>
                  ✗ Reject Pay
                </button>
              )}
              {order.status === "READY" &&
              ...
              <button
                className="sm"
                onClick={() => {
                  setModal({ type: "eta", order });
                  setEtaValue(order.delivery?.etaMinutesOverride ?? "");
                }}>
                ⏱ ETA
              </button>
      ...
      {modal?.type === "eta" && (
        <Modal
          title={`Override ETA — ${modal.order.orderNo}`}
          onClose={closeModal}>
          <label>
            ETA (minutes)
            <input
              ...
              onClick={() =>
                act(
                  () =>
                    updateEta({
                      id: modal.order._id,
                      etaMinutesOverride: Number(etaValue),
                    }).unwrap(),
                  "ETA updated",
                ).then(closeModal)
              }>
      ...
      {modal?.type === "cancel" && (
        <Modal
          title={`Cancel Order — ${modal.order.orderNo}`}
          onClose={closeModal}>
          <label>
            Reason
            <select
          ...
          {cancelForm.reason === "Other" && (
            <label style={{ marginTop: "0.75rem" }}>
              Custom reason
              <input
                value={cancelForm.reasonText}
                onChange={(e) =>
                  setCancelForm({ ...cancelForm, reasonText: e.target.value })
                }
                ...
                act(
                  () =>
                    cancelOrder({
                      ...
                      reasonText: cancelForm.reasonText || cancelForm.reason,
                    }).unwrap(),
                  "Order cancelled",
                ).then(closeModal)
              }>
      ...
      {modal?.type === "reject" && (
        <Modal
          title={`Reject Order — ${modal.order.orderNo}`}
          onClose={closeModal}>
          <label>
            ...
            <input
              ...
              placeholder="e.g. Kitchen closed"
              autoFocus
            />
          </label>
          <div className="actions" style={{ marginTop: "1rem" }}>
              ...
              onClick={() =>
                act(
                  () =>
                    rejectOrder({
                      id: modal.order._id,
                      reason: rejectOrderReason || "Order rejected",
                    }).unwrap(),
                  "Order rejected",
                ).then(closeModal)
              }>
      ...
      {modal?.type === "rejectPayment" && (
        <Modal
          title={`Reject Payment — ${modal.order.orderNo}`}
          onClose={closeModal}>
          <label>
            Reason for rejection
            <input
