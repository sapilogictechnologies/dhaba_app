...
const statusLabel = { free: 'FREE', occupied: 'OCCUPIED', inactive: 'INACTIVE' };
const statusColor = { free: '#16a34a', occupied: '#d97706', inactive: '#78716c' };
...
const TablesPage = () => {
  ...
  const addTable = async (e) => {
    ...
    await act(
      () => createTable({ tableNumber: Number(newTable.tableNumber), capacity: Number(newTable.capacity) }).unwrap(),
  ...
  const printAllQR = () => {
    const win = window.open('', '_blank');
    const appUrl = window.location.origin;
    const items = tables.filter((t) => t.isActive && t.qrCodeUrl).map((t) => `
      <div class="qr-item">
  ...
  return (
    <section className="shell">
      ...
      <div className="panel">
        ...
        <form onSubmit={addTable}>
          <div className="form-grid">
            <label>Table Number
              <input type="number" value={newTable.tableNumber} min="1"
                onChange={(e) => setNewTable({ ...newTable, tableNumber: e.target.value })}
                placeholder="e.g. 11" required />
            </label>
            <label>Capacity
              <input type="number" value={newTable.capacity} min="1"
                onChange={(e) => setNewTable({ ...newTable, capacity: e.target.value })} />
            </label>
            <div style={{ display: 'flex', alignItems: 'flex-end' }}>
              <button type="submit" className="primary">Add Table</button>
            </div>
          </div>
        </form>
      </div>
      ...
      <div className="cards" style={{ marginBottom: '1rem' }}>
        <div className="metric"><span>Total</span><strong>{tables.length}</strong></div>
        <div className="metric"><span style={{ color: '#16a34a' }}>Free</span><strong style={{ color: '#16a34a' }}>{tables.filter((t) => getTableStatus(t) === 'free').length}</strong></div>
        <div className="metric"><span style={{ color: '#d97706' }}>Occupied</span><strong style={{ color: '#d97706' }}>{tables.filter((t) => getTableStatus(t) === 'occupied').length}</strong></div>
        <div className="metric"><span>Inactive</span><strong>{tables.filter((t) => !t.isActive).length}</strong></div>
      </div>
        ...
        {tables.map((table) => {
          ...
          return (
            <div key={table._id} className={`table-card ${status}`}>
              ...
              <div className="table-status" style={{ color: statusColor[status] }}>{statusLabel[status]}</div>
              <div style={{ fontSize: '0.78rem', color: '#78716c' }}>Cap: {table.capacity}</div>

              {table.qrCodeUrl && (
                <img className="qr" src={table.qrCodeUrl} alt={`QR Table ${table.tableNumber}`} />
              )}

              <div style={{ display: 'flex', gap: '0.3rem', flexWrap: 'wrap', justifyContent: 'center', marginTop: '0.3rem' }}>
                <button className="sm" onClick={() => printQR(table)} title="Print QR">🖨</button>
                <button className="sm" onClick={() => copyLink(table)} title="Copy order link">🔗</button>
                <button className="sm" onClick={() => act(() => generateTableQr(table._id).unwrap(), 'New QR generated')}>
                  ↺ QR
                </button>
                <button className="sm" onClick={() => act(() => updateTable({ id: table._id, isActive: !table.isActive }).unwrap(), table.isActive ? 'Deactivated' : 'Activated')}>
                  {table.isActive ? 'Deactivate' : 'Activate'}
                </button>
                <button className="sm danger" onClick={() => {
                  if (window.confirm(`Delete Table ${table.tableNumber}?`))
                    act(() => deleteTable(table._id).unwrap(), 'Table deleted');
                }}>Del</button>
              </div>
            </div>
          );
      ...
      {tables.length === 0 && !isLoading && (
        <div className="empty-state"><div className="icon">🪑</div><p>No tables yet. Add one above.</p></div>
      )}
    </section>
  );
};
