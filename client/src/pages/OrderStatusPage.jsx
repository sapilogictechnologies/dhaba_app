...
const STATUS_STEPS = [
  ...
  },
  {
    ...
    icon: "💳",
    desc: "Verifying your UPI payment",
  },
  {
    status: "ACCEPTED",
    label: "Order Accepted",
    icon: "✅",
    desc: "Confirmed and in the queue",
  },
  {
    status: "PREPARING",
    label: "Preparing",
  ...
  },
  {
    ...
    label: "Ready!",
    icon: "🛎",
    desc: "Your order is ready to go",
  },
  {
  ...
  },
  {
    ...
    label: "Delivered",
    icon: "🏠",
    desc: "Delivered! Enjoy your meal 🎉",
  },
  {
    status: "PICKED_UP",
    label: "Picked Up",
    icon: "🛍",
    desc: "Collected! Enjoy your meal 🎉",
  },
  {
    status: "COMPLETED",
    label: "Served",
...
const STATUS_NOTIFICATIONS = {
  UNDER_REVIEW: {
    msg: "⏳ Payment under review — we'll confirm shortly",
    type: "warn",
  },
...
const getStepsForOrder = (order) => {
  ...
  if (dt === "DELIVERY")
    return STATUS_STEPS.filter(
      (s) => !["PICKED_UP", "COMPLETED"].includes(s.status),
    );
  if (dt === "PICKUP")
    return STATUS_STEPS.filter(
      (s) => !["OUT_FOR_DELIVERY", "DELIVERED", "COMPLETED"].includes(s.status),
    );
  return STATUS_STEPS.filter(
    (s) =>
      !["UNDER_REVIEW", "OUT_FOR_DELIVERY", "DELIVERED", "PICKED_UP"].includes(
        s.status,
      ),
  );
};
...
const SOURCE_LABELS = {
  WALKIN_TABLE: "Dine-in",
  TAKEAWAY_COUNTER: "Takeaway",
  PHONE_MANUAL: "Phone Order",
  QR_TABLE: "Table Order",
  ONLINE_PICKUP: "Pickup",
  ONLINE_DELIVERY: "Home Delivery",
};
...
const OrderStatusPage = () => {
  ...
  const { data, isLoading, error, refetch } = useGetPublicOrderQuery(orderNo, {
  ...
  if (isLoading)
    return (
      <div
        ...
        <span
          className="spinner"
          style={{ width: "2.5rem", height: "2.5rem", borderWidth: 3 }}></span>
      </div>
    );
    ...
    <div
      ...
      <div className="public-page" style={{ paddingTop: "1.25rem" }}>
        ...
        <div
          ...
          <div style={{ fontSize: "2.75rem", marginBottom: "0.4rem" }}>
            {currentStep?.icon || "📋"}
          </div>
          <div
            style={{
        ...
        {order.payment?.paymentStatus &&
          order.payment.paymentStatus !== "UNPAID" && (
            <div
              className={`alert ${order.payment.paymentStatus === "PAID" ? "alert-success" : order.payment.paymentStatus === "UNDER_REVIEW" ? "alert-warn" : "alert-error"}`}
              style={{ marginBottom: "1rem" }}>
              {order.payment.paymentStatus === "PAID" && "✓ Payment Confirmed"}
              {order.payment.paymentStatus === "UNDER_REVIEW" &&
                "⏳ Payment Under Review — we'll confirm shortly"}
              {order.payment.paymentStatus === "REJECTED" &&
                `❌ Payment Rejected${order.payment.rejectionReason ? ": " + order.payment.rejectionReason : ""}`}
            </div>
          )}
                    ...
                    <div style={{ flex: 1 }}>
                      <div
                        style={{
                          ...
                          fontSize:
                            state === "current" ? "0.95rem" : "0.875rem",
          ...
          <ul className="order-items">
            {(order.kotItemsSnapshot?.length
              ...
              const price =
                item.priceSnapshot ??
                order.items.find(
                  (x) => String(x.itemId) === String(item.itemId),
                )?.priceSnapshot;
              return (
        ...
        <div
          ...
          <Link to="/order">
            <button
              ...
              style={{ borderRadius: 10, padding: "0.75rem", fontWeight: 700 }}>
              + Place New Order
            </button>
          </Link>
        </div>
